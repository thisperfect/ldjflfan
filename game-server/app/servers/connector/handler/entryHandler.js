...
var entryService = require('../../../util/entryService');
...
var bindSession = function(self, session, uid){
...
handler.enter = function(msg, session, next) {
	var self = this;
	var token = msg.token;
	var sessionService = self.app.get('sessionService');		// 用来管理session的一个对象
	var loginIP = self.app.sessionService.getClientAddressBySessionId(session.id).ip;
		...
		entryService.auth(uid,deviceId,loginIP,function (data) {
			...
			entryService.enter(uid,loginIP, function (data) {							//登录服务,玩家信息获取
                ...
                self.app.rpc.world.worldRemote.add(session,uid, function(res){		            //进入游戏世界
                    ...
                    var vipLevel= parseInt(playerInfo['vipLevel']);					                    //VIP玩家推送喇叭消息
                    if( vipLevel > 0){

                        var nickname = value['nickname'] ? value['nickname']:'玩家_'+uid;
                        var content ='欢迎ID为'+ uid +'的玩家【<color=#ffff00>VIP'+vipLevel+'</color>】用户【<color=#ffff00> '+nickname+' </color>】回到了游戏,祝您游戏愉快!';
